...
# Numerosity data - lineare regression

```{r}
"1) Congruent"
drt.congruent.digits <- numerosity.dat %>%
...
"2) Incongruent"
drt.incongruent.digits <- numerosity.dat %>%
  dplyr::filter(Congruent == "Incong", ppid != 45, ppid != 5, outlier == FALSE) %>%
  dplyr::group_by(number, key_response, ppid) %>%
  dplyr::summarise(meanMS = mean(ms)) %>%
  dplyr::ungroup() %>%
  tidyr::pivot_wider(names_from = key_response, values_from = meanMS) %>%
...
"saving plots"
g <- arrangeGrob(drt.congruent.plot, drt.incongruent.plot, nrow = 1)
  ggsave(here::here("Parity_DvN_SNARC/Plots/Numerosity_Linear.tiff"), g,  width = 15, height = 7, units = 'cm', dpi = 900, type = 'cairo')
```
